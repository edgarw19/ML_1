...
//  printAccel(); // Print "A: ax, ay, az"
  ...
  //If rider is on, calculate the right speed
    ...
    //If not, drive the motor
      ...
      double speedFraction = (maxWeight - .55*RIDERWEIGHT)/(.25*RIDERWEIGHT);
      Serial.println("SPEED FRACTION:" + speedFraction);
      if (speedFraction > 1) speedFraction = 1;
      speedFraction = pow(speedFraction, 15);
      int extraPulseWidth = speedFraction * (DUTYRANGE-constStart)/(DUTYRANGE) * DUTYRANGE + constStart;
